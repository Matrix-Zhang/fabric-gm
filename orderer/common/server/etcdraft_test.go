...
func TestSpawnEtcdRaft(t *testing.T) {
	...
	configtxgen, err := gexec.Build("github.com/Matrix-Zhang/fabric-gm/common/tools/configtxgen")
	gt.Expect(err).NotTo(HaveOccurred())

	// Build the orderer binary
	orderer, err := gexec.Build("github.com/Matrix-Zhang/fabric-gm/orderer")
	gt.Expect(err).NotTo(HaveOccurred())
